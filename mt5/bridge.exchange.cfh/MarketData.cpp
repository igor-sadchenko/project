...
namespace cfh {
	...
	void MarketData::FixMarketDataRequestReject(const libfix::_Y__* tagY)
	{
		...
		shared::marketdata::EnCFHMDRejectReason Reason = shared::marketdata::EnCFHMDRejectReason(tagY->_281.empty() ? '0' : tagY->_281[0]);

		m_MDRequestCacheQ.pop(
			std::stoull(tagY->_262, nullptr, 10),
			[&](const auto&, auto& id)
			{
				...
				Global::GetSetup()->m_ProductQ.pop(
					...
					[&](const auto&, auto& inst)
					{
						...
						switch (Reason)
						{
						case shared::marketdata::EnCFHMDRejectReason::EN_CFH_MD_REJECT_REASON_DUPLICATE_MDREQID:
						{
							strReason = "EN_CFH_MD_REJECT_REASON_DUPLICATE_MDREQID";
						}break;
						case shared::marketdata::EnCFHMDRejectReason::EN_CFH_MD_REJECT_REASON_INSUFFICIENT_PERMISSIONS:
						{
							strReason = "EN_CFH_MD_REJECT_REASON_INSUFFICIENT_PERMISSIONS";
						}break;
						case shared::marketdata::EnCFHMDRejectReason::EN_CFH_MD_REJECT_REASON_UNKNOWN_SYMBOL:
						{
							strReason = "EN_CFH_MD_REJECT_REASON_UNKNOWN_SYMBOL";
						}break;
						case shared::marketdata::EnCFHMDRejectReason::EN_CFH_MD_REJECT_REASON_UNSUPPORTED_SUBSCRIPTIONREQUESTTYPE:
						{
							strReason = "EN_CFH_MD_REJECT_REASON_UNSUPPORTED_SUBSCRIPTIONREQUESTTYPE";
						}break;
						default:
						{
							strReason = "UNKNOWN";
	...
	int MarketData::FixMarketDataRequest()
	{
		...
		Global::GetSetup()->m_ProductQ.iterate(
			...
			{
				libfix::_V__ v;
				/*v._262 = shared::SystemIDType::SystemOrderIDToStr(shared::SystemIDType::SystemOrderIDMade(0xB00000000));*/
				...
				v.__146.emplace_back(libfix::_V__::_146{ "","",inst.Symbol.symbol });
				...
				{
					Global::Log(sk::EnLogType::EN_LOG_TYPE_ERROR)->error("报价请求失败(Symbol({}))", inst.Symbol.symbol);
				}
	...
	{
		if (!tagW || tagW->_55.empty()) return;

		shared::marketdata::MarketData md;
		auto found = Global::GetSetup()->m_SymbolProductQ.search(
			tagW->_55,
